    ...
    with files.ChDir(abs_repository_path) as _:
      ...
      if output_match:
        ...
        if head_branch != '(unknown)':
          ...
          try:
            # At one point, git's default behavior for upstream branches
            # changed, and the --set-upstream-to flag was introduced. Before
            # that flag was introduced, the default is what we set it to, here.
            CheckGitVersion(_UPSTREAM_MIN)
            subprocess.check_call(
                ['git', 'branch',
